...
#ifdef HAVE_SBRK
  char *start_sbrk = (char *) sbrk (0);//进程虚拟地址到内存地址映射
#endif

#if defined (HAVE_SETLOCALE) && defined (HAVE_LC_MESSAGES)
  setlocale (LC_MESSAGES, ""); // 初始化本地信息
#endif
#if defined (HAVE_SETLOCALE)
  setlocale (LC_CTYPE, ""); // 语言符号分类
#endif
  bindtextdomain (PACKAGE, LOCALEDIR);
  textdomain (PACKAGE);
//根据不同语言去在/usr/share/locale 目录下进行寻找
  program_name = argv[0]; // 什么名字
  //例如 ld-new , 绝对路径
...
//设置配置项
  ...
  /* SVR4 linkers seem to set DT_INIT and DT_FI
/ * SVR4链接器似乎设置了DT_INIT和DT_FINI based on magic _init
/ *基于magic _init的SVR4链接器似乎设置了DT_INIT和DT_FINI  and _fini symbols.  We are compatible.  */
//*基于_init的SVR4和_fini符号。我们是兼容的。 * /
  ...
  ldfile_add_arch ("");
  emulation = get_emulation (argc, argv); // 类似于获取当前平台的属性 target
  ldemul_choose_mode (emulation);
  default_target = ldemul_choose_target (argc, argv);
  config.maxpagesize = bfd_emul_get_maxpagesize (default_target);
  config.commonpagesize = bfd_emul_get_commonpagesize (default_target);
  lang_init (); // 解析脚本语言的初始化
  ldexp_init ();
  ldemul_before_parse ();
